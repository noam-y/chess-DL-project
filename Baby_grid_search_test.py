...
def main():
    ...
    test_combinations = [(1, 'none', False, True), (2, '50_50', False, False), (2, 'uniform', True, True),
                         (3, 'none', False, False), (3, '50_50', True, True)]
    all_games = ['game2', 'game4', 'game6', 'game7']
    results = []

    for combination_idx, (heads, sampling, use_triplet, freeze) in enumerate(test_combinations, 1):
        config_name = f"H{heads}_S-{sampling}_T-{use_triplet}_F-{freeze}"
        print(f"\n{'=' * 60}\nModel {combination_idx}/5: {config_name}\n{'=' * 60}")
